...
public class InventarioDiarioService {
    ...
    public static void validarFechaInventario(LocalDate fecha) {
        if (fecha == null) {
    ...
    public static void validarFechaCajaInventario(LocalDate fechaInventario, LocalDate fechaCaja) {
        if (fechaInventario == null || fechaCaja == null) {
            throw new DatoInvalidoException("Las fechas son obligatorias");
        }
        if (!fechaInventario.equals(fechaCaja)) {
            throw new ConflictoException("No se puede registrar inventario en una fecha diferente a la de la caja activa. Caja: " + fechaCaja + ", Inventario: " + fechaInventario);
        }
    }

    @Transactional
    public List<InventarioDiario> registrarLote(LocalDate fecha, List<InventarioItemRequest> items) {
        validarFechaInventario(fecha);
